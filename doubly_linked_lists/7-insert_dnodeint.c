...
dlistint_t *insert_dnodeint_at_index(dlistint_t **h, unsigned int idx, int n)
{
	...
	while (current_node)
	{
		...
		if (i == idx - 1)
		{
			...
			if (new_node == NULL)
				return (NULL);
			new_node->n = n; /* donne valeur membre n */
			new_node->next = current_node->next; /* donne valeur membre next */
			new_node->prev = current_node; /* donne valeur membre prev */
			current_node->next->prev = new_node;
			current_node->next = new_node; /* donne valeur membre next */
